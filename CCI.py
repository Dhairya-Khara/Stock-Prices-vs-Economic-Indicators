...
def get_list_of_cci_rates():
    list_so_far = []
    adf = pd.read_excel('CCI.xlsx')
    for index, row in adf.iterrows():
        list_so_far.append(row['CCI'])
    return list_so_far


# STEP 6 - Linear Regression.
def get_r_squared_cci():
    correlation_matrix = np.corrcoef(x, y)
    correlation_xy = correlation_matrix[0, 1]
    r_squared_cci = correlation_xy ** 2
    return r_squared_cci
...
def return_info_to_graph_cci(stock_ticker):
    ...
    list_of_cci = get_list_of_cci_rates()  # STEP 5
    x = np.array(list_of_cci)
