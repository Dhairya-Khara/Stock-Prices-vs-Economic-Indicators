...
# Step 5 (2) - Get independent variables in a list, unemployed_per_opening in this case.
def get_list_of_unemployed_per_opening():
    list_so_far = []
    adf = pd.read_excel('unemployed_per_opening.xlsx', sheet_name='Sheet 1')
...
def return_unemployed_per_opening_to_graph(stock_ticker):
    ...
    list_of_unemployed_per_opening = get_list_of_unemployed_per_opening()  # Step 5 (2)
    x = np.array(list_of_unemployed_per_opening)
    y = np.array(list_of_median_values)
    return x, y
